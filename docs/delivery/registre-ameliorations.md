...
| POL-045 | P1 | Aucune limitation de débit sur l'API. **Devenu prioritaire** : la route de connexion est désormais publique, donc attaquable par force brute. Argon2 rend chaque essai coûteux pour l'attaquant comme pour le serveur — sans limitation, une rafale suffit à saturer l'API. |
